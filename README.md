...
- `Обогнал_всего_%` — процент менеджеров с меньшим приростом (глобально)
- `Обогнали_меня_всего_%` — процент менеджеров с большим приростом (глобально)
...
Расчет процентилей может выполняться с фильтром (например, `">=0"` для учета только неотрицательных приростов), который задается независимо от фильтра для вывода в SPOD.

**Формула процентиля**:
```
Обогнал_% = (количество менеджеров с меньшим значением + 0.5 * (количество равных - 1)) / (общее количество - 1) * 100
```
...
- `columns`: маппинг alias ↔ оригинальный заголовок в Excel
- `sheet`: имя листа по умолчанию

### 6.2 `filters` — правила очистки
- `drop_rules`: список правил для удаления строк с запрещенными значениями по колонкам

### 6.3 `defaults` — заглушки
...
- `percentile_filter`: фильтр для расчета процентилей (`">=0"` или `"all"`)
- `variants`: список вариантов выгрузки СПОД, каждый содержит:
  ...
  - `value_column`: колонка со значениями для выгрузки
...
- `active_variant`: выбранный активный вариант (`"variant_1"`, `"variant_2"` или `"variant_3"`)
