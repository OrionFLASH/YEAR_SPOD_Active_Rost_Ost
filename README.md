...
| 2.1.1 | 2025-12-02 | Исправлена критическая ошибка в логике определения итогового ТН и КМ: теперь колонки переименовываются перед join, что гарантирует правильные имена колонок (`ВКО_curr`, `ВКО_prev`, `ВКО_prev2`) и корректную работу `combine_first` с приоритетом T-0 → T-1 → T-2. Теперь если клиент есть только в T-0, итоговый ТН и КМ правильно берутся из T-0, а не из значений по умолчанию. |
