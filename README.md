...
- `use_files_count`: количество файлов для расчета
  - `2` — расчет по двум файлам (T-0 и T-1), требуется наличие файлов `current` и `previous`
  - `3` — расчет по трем файлам (T-0, T-1 и T-2), требуется наличие файлов `current`, `previous` и `previous2`
