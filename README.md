...
- На вход поступают два или три XLSX-файла из каталога `IN`:
...
**Настройки расчета процентилей** (в блоке `variants` для каждого варианта):
...
- `active_percentile_variant`: выбранный активный вариант для расчета процентиля (`"variant_1"`, `"variant_2"` или `"variant_3"`)
  - Может отличаться от `active_variant`, что позволяет использовать разные варианты для основного расчета и расчета процентиля
  - Если не указан, используется `active_variant`
- `variant_1`: настройки варианта 1 (по КМ, без ТБ)
  - `name`: имя варианта
  - `key_mode`: `"manager"`
  - `include_tb`: `False`
  - `summary_sheet_name`: имя листа свода (не используется)
  - `percentile_sheet_name`: имя листа процентилей (не используется)
  - `percentile_type`: тип процентиля для расчета (`"обогнал"` или `"обогнали"`)
    - `"обогнал"` — рассчитывается процент КМ с меньшим результатом (кого я обогнал)
    - `"обогнали"` — рассчитывается процент КМ с большим результатом (кто меня обогнал)
  - `percentile_group_by`: уровень группировки для расчета процентиля
    ...
    - `"tb_and_gosb"` — среди КМ с тем же ТБ и ГОСБ одновременно
  - `percentile_filter`: фильтр для данных при расчете процентилей (`">=0"`, `">0"`, `"all"` и т.д.)
    - `">=0"` — только неотрицательные значения
    - `">0"` — только положительные значения
    - `"all"` — все значения
- `variant_2`: настройки варианта 2 (по ИНН, без ТБ)
  - `name`: имя варианта
  - `key_mode`: `"client"`
  - `include_tb`: `False`
  - `summary_sheet_name`: имя листа свода (не используется)
  - `percentile_sheet_name`: имя листа процентилей (не используется)
  - `percentile_type`: тип процентиля для расчета (см. `variant_1`)
  - `percentile_group_by`: уровень группировки для расчета процентиля (см. `variant_1`)
  - `percentile_filter`: фильтр для данных при расчете процентилей (см. `variant_1`)
- `variant_3`: настройки варианта 3 (по ИНН, с ТБ)
  - `name`: имя варианта
  - `key_mode`: `"client"`
  - `include_tb`: `True`
  - `summary_sheet_name`: имя листа свода (не используется)
...
### 6.7 `report_layout` — управление выводом листов
