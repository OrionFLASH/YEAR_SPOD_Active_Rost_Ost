...
-
-1. **ВКО, без ТБ, КМ по каждому файлу** — для каждого ВКО в T-0 и T-1 ищется КМ с максимальным вкладом; прирост распределяется по соответствующим КМ.
-2. **ВКО, с ТБ, КМ по каждому файлу** — дополнительно фиксируется доминантный ТБ у выбранного КМ; прирост учитывается только при совпадении ТБ.
-3. **ВКО, без ТБ, последний КМ** — прирост ВКО всегда закрепляется за актуальным КМ (T-0 → T-1 → заглушка).
-4. **ВКО, с ТБ, последний КМ+ТБ** — определяется доминантный ТБ последнего КМ; прирост учитывается только для этой пары.
-5. **ИНН, без ТБ, КМ по каждому файлу** — аналог пункта 1, но ключём является клиент.
-6. **ИНН, с ТБ, КМ по каждому файлу** — дополнительно учитывается доминантный ТБ у каждого КМ.
-7. **ИНН, без ТБ, последний КМ** — клиентский прирост всегда идёт на последнего КМ.
-8. **ИНН, с ТБ, последний КМ+ТБ** — прирост клиента попадает в итог только если он сопоставлен с доминантным ТБ последнего КМ.
-
